...
namespace P2PFinalChat
{
    ...
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            // Vẽ giao diện lần đầu tiên (để hiển thị Info ngay lập tức)
            RedrawConsole();
        ...
        private static async Task ConnectToPeer(string ip, int port)
        {
            ...
            {
                // Có thể in lỗi ra System log nếu muốn
        ...
        // --- [QUAN TRỌNG] VẼ LẠI MÀN HÌNH (GỒM CẢ PHẦN INFO) ---
        private static void RedrawConsole()
        {
            lock (_lock)
            {
                ...
                Console.WriteLine(" - Ket noi: /connect <IP> <Port>");
                Console.WriteLine(" - Sua tin: /edit <STT_Vang> <noi dung moi>");
        ...
        private static void ShowNeighbors() { lock (_lock) Console.WriteLine($"\n[SYSTEM] Dang ket noi voi {_neighbors.Count} nut."); }
    }
}