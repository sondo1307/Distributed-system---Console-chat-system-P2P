...
namespace P2PFinalJson
{
    ...
    class Program
    {
                        ...
                        try
                        {
                            using (var client = new TcpClient())
                            {
                                ...
                                if (await Task.WhenAny(connectTask, timeoutTask) == connectTask && client.Connected)
                                {
                                    ...
                                    try { var ping = new ChatPacket { Type = PacketType.Ping }; using var w = new StreamWriter(client.GetStream()) { AutoFlush = true }; await w.WriteLineAsync(JsonSerializer.Serialize(ping)); } catch { }
                                }
                                else friend.IsOnline = false;
                            }
                        }
                        catch { friend.IsOnline = false; }
                    }
                }
        ...
        static async Task Screen_Login()
        {
            while (true)
            {
                Console.Clear(); Console.ForegroundColor = ConsoleColor.Cyan; Console.WriteLine("=== P2P CHAT ==="); Console.ResetColor();
                ...
                Console.Write("Nhập tên: "); string name = Console.ReadLine(); if (!name.Any(char.IsLetter)) continue;
                Console.Write("Nhập port (VD: 9000): "); if (int.TryParse(Console.ReadLine(), out int port)) { _currentUser = new UserConfig { Username = name, Port = port }; JsonManager.SaveConfig(_currentUser); return; }
            }
        }
        ...
        static string Screen_Dashboard()
        {
            ...
            while (true)
            {
                ...
                Console.WriteLine("NEUTRAL COMMAND: [N] New group chat | [J] Join | [RESET] Reset all data | [Ctrl+R] Refresh");
                Console.WriteLine("PHONEBOOK COMMAND: [F] Add friend (Invite) | [L] List friends");
                ...
                for (int i = 0; i < sessions.Count; i++) Console.WriteLine($"[{i + 1}] {sessions[i].Name} (ID: {sessions[i].SessionId.Substring(0, 4)}...)");
                Console.Write("> ");
                ...
                if (_pendingFriendReq != null)
                {
                    if (input.Equals("Y", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        if (parts.Length == 2) { JsonManager.AddFriend(_pendingFriendReq.SenderName, parts[0], int.Parse(parts[1])); Console.WriteLine($"[SUCCESS] Added {_pendingFriendReq.SenderName} to contacts."); _ = SendFriendResponse(parts[0], int.Parse(parts[1]), true); }
                        _pendingFriendReq = null; Thread.Sleep(1500); continue;
                    }
                    else if (input.Equals("N", StringComparison.OrdinalIgnoreCase)) { Console.WriteLine("[INFO] Ignored friend request."); _pendingFriendReq = null; Thread.Sleep(1000); continue; }
                }
                ...
                if (input.Equals("F", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    Console.WriteLine("Sending request..."); _ = SendFriendRequest(ip, p);
                    Console.WriteLine("Request sent! Waiting for approval..."); Thread.Sleep(2000);
                }
                else if (input.Equals("L", StringComparison.OrdinalIgnoreCase))
                {
                    Console.Clear(); Console.WriteLine("--- List friends ---");
                    if (_myFriends.Count == 0) Console.WriteLine("(You have zero friend)");
                    foreach (var f in _myFriends) { Console.Write($"- {f.Name} ({f.Ip}:{f.Port}): "); if (f.IsOnline) { Console.ForegroundColor = ConsoleColor.Green; Console.WriteLine("ONLINE"); } else { Console.ForegroundColor = ConsoleColor.DarkGray; Console.WriteLine("OFFLINE"); } Console.ResetColor(); }
                    Console.WriteLine("\nPress Enter to back to DASHBOARD..."); Console.ReadLine();
                }
                else if (input.StartsWith("N ", StringComparison.OrdinalIgnoreCase)) { string chatName = input.Substring(2).Trim(); string newId = GenerateShortId(); JsonManager.UpsertSession(newId, string.IsNullOrEmpty(chatName) ? "New Chat" : chatName); return newId; }
                else if (input.Equals("RESET", StringComparison.OrdinalIgnoreCase)) { Console.Write("CONFIRM DELETING DATA? (Y/N): "); if (Console.ReadLine()?.ToUpper() == "Y") { JsonManager.DeleteAllData(); return "RESET_APP"; } }
                else if (input.Equals("J", StringComparison.OrdinalIgnoreCase)) { Console.Write("Input: <IP> <Port> <RoomID>: "); string joinCmd = Console.ReadLine(); var parts = joinCmd.Split(' '); if (parts.Length == 3 && int.TryParse(parts[1], out int p)) { string targetId = parts[2]; JsonManager.UpsertSession(targetId, "Joining..."); _ = ConnectAndJoin(parts[0], p, targetId); Console.WriteLine("Connecting..."); Thread.Sleep(1000); return targetId; } }
                else if (int.TryParse(input, out int idx) && idx > 0 && idx <= sessions.Count) return sessions[idx - 1].SessionId;
            }
        }

        // --- SCREEN 3: CHAT ROOM (ĐÃ SỬA LẠI /INVITEFRIEND) ---
        static async Task Screen_ChatRoom(string sessionId)
        {
            ...
            while (true)
            {
                ...
                // [SỬA ĐỔI] Tính năng invite friend theo danh sách số
                if (input.Trim() == "/invitefriend")
                {
                    // Lấy snapshot danh sách để tránh lỗi thread
                    var availableFriends = _myFriends.ToList();

                    if (availableFriends.Count == 0)
                    {
                        Console.WriteLine("[SYSTEM] Contact list is empty. Please add friends in Dashboard [F].");
                    }
                    else
                    {
                        ...
                        for (int i = 0; i < availableFriends.Count; i++)
                        {
                            ...
                            ConsoleColor color = f.IsOnline ? ConsoleColor.Green : ConsoleColor.DarkGray;
                        ...
                        if (int.TryParse(choice, out int idx) && idx > 0 && idx <= availableFriends.Count)
                        {
                            var target = availableFriends[idx - 1];
                            await ConnectAndJoin(target.Ip, target.Port, sessionId);
                            Console.WriteLine($"[SYSTEM] Sending invite to {target.Name}...");
                        }
                        else if (choice != "0")
                        {
                    ...
                    // Vẽ lại giao diện để xóa các dòng lựa chọn
                    Thread.Sleep(1500);
                    RedrawChat(sessionId);
                    continue;
                }
                ...
                else if (input.StartsWith("/invite ")) { var p = input.Split(' '); if (p.Length == 3) await ConnectAndJoin(p[1], int.Parse(p[2]), sessionId); }
                else if (input.StartsWith("/edit ")) { var parts = input.Split(' ', 3); if (parts.Length == 3 && int.TryParse(parts[1], out int index)) HandleEditCommand(index, parts[2], sessionId); }
                else if (input.StartsWith("/delete ")) { var parts = input.Split(' '); if (parts.Length == 2 && int.TryParse(parts[1], out int index)) HandleDeleteCommand(index, sessionId); }
        ...
        static async Task ConnectAndJoin(string ip, int port, string targetSessionId) { try { TcpClient c = new TcpClient(); await c.ConnectAsync(ip, port); lock (_uiLock) _neighbors.Add(c); string myGroupName = JsonManager.GetSessionName(targetSessionId); var packet = new ChatPacket { Id = Guid.NewGuid().ToString(), SessionId = targetSessionId, GroupName = myGroupName, Type = PacketType.Invite, SenderName = _currentUser.Username, SenderInfo = $"{GetLocalIP()}:{_currentUser.Port}", Content = "Hello! I am added you to this group", Timestamp = DateTime.Now }; var w = new StreamWriter(c.GetStream()) { AutoFlush = true }; await w.WriteLineAsync(JsonSerializer.Serialize(packet)); _ = Task.Run(() => HandleClient(c)); } catch { Console.WriteLine($"[ERROR] Connection error to {ip}:{port}"); } }
        static void HandleEditCommand(int index, string newContent, string sessionId) { if (index > 0 && index <= _currentViewMap.Count) { var target = _currentViewMap[index - 1]; if (target.SenderName == _currentUser.Username) { var p = new ChatPacket { Id = Guid.NewGuid().ToString(), TargetId = target.Id, SessionId = sessionId, Type = PacketType.Edit, SenderName = _currentUser.Username, SenderInfo = $"{GetLocalIP()}:{_currentUser.Port}", Content = newContent, Timestamp = DateTime.Now }; ProcessPacket(p); Broadcast(p); } else Console.WriteLine("[ERROR] You ONLY allow to edit your message."); } }
        static void HandleDeleteCommand(int index, string sessionId) { if (index > 0 && index <= _currentViewMap.Count) { var target = _currentViewMap[index - 1]; if (target.SenderName == _currentUser.Username) { var p = new ChatPacket { Id = Guid.NewGuid().ToString(), TargetId = target.Id, SessionId = sessionId, Type = PacketType.Delete, SenderName = _currentUser.Username, SenderInfo = $"{GetLocalIP()}:{_currentUser.Port}", Timestamp = DateTime.Now }; ProcessPacket(p); Broadcast(p); } else Console.WriteLine("[ERROR] You ONLY allow to delete your message."); } }
