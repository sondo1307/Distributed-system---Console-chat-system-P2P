    ...
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                string selectedSession = await Screen_Dashboard(); // Sửa thành await vì Dashboard giờ là async
            ...
            while (true)
            {
                ...
                else if (input.Equals("J", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (parts.Length == 3 && int.TryParse(parts[1], out int p))
                    {
                        ...
                        // Chờ kết nối, nếu thành công thì mới vào
                        ...
                        else
                        {
                            ...
                            Console.WriteLine("[ERROR] Connection failed! Room not found or User offline.");
                            Console.ResetColor();
                            Thread.Sleep(2000);
                            // Vòng lặp while(true) sẽ quay lại Dashboard, không vào ChatRoom
                        }
                    }
                }
        ...
        static async Task Screen_ChatRoom(string sessionId)
        {
            ...
            while (true)
            {
                ...
                if (input.Trim() == "/invitefriend")
                {
                    ...
                    {
                        Console.WriteLine("\n--- SELECT FRIEND ---");
                        for (int i = 0; i < availableFriends.Count; i++) { var f = availableFriends[i]; string st = f.IsOnline ? "ONLINE" : "OFFLINE"; Console.Write($"[{i + 1}] "); if (f.IsOnline) Console.ForegroundColor = ConsoleColor.Green; else Console.ForegroundColor = ConsoleColor.DarkGray; Console.WriteLine($"{f.Name} ({st})"); Console.ResetColor(); }
                        Console.Write("Enter number: "); string c = Console.ReadLine();
                        if (int.TryParse(c, out int idx) && idx > 0 && idx <= availableFriends.Count)
                        {
                            ...
                            if (!ok) Console.WriteLine($"[ERROR] Cannot connect to {t.Name}.");
                        }
                    }
        ...
        // [QUAN TRỌNG] Đã sửa hàm này để trả về bool
        static async Task<bool> ConnectAndJoin(string ip, int port, string targetSessionId)
        {
            try
            {
                ...
                var timeoutTask = Task.Delay(2000); // 2 giây timeout
